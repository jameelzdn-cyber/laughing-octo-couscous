...
def show_subscription_requirements(chat_id, user_id, lang):
    ...
    if channels:
        for index, ch in enumerate(channels, start=1):
            ch_title = get_channel_name(ch['channel_id'], "اضغط هنا للاشتراك 📢")
...
def verify_channels_btn(call):
    ...
    if shares < 3:
        ...
        msg_share = "⚠️ يجب عليك مشاركة البوت مع الأصدقاء لتفعيل الأقسام." if lang=='ar' else "⚠️ You must share the bot with friends." if lang=='en' else "⚠️ پێویستە بۆتەکە لەگەڵ هاوڕێیاندا هاوبەش بکەيت."
