...
# --- لوحة التعديل الشاملة الذكية ---
@bot.callback_query_handler(func=lambda call: call.data == "adm_edit_hub")
def adm_edit_hub_main(call):
    delete_msg_safe(call.message.chat.id, call.message.message_id)
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📁 تعديل أسماء الأقسام الرئيسية والفرعية", callback_data="hub_edit_cats"))
    markup.add(types.InlineKeyboardButton("✉️ تعديل محتويات الرسائل وروابط اليوتيوب", callback_data="hub_edit_msgs"))
    markup.add(types.InlineKeyboardButton("⬅️ العودة للوحة الإدارة", callback_data="back_to_admin"))
    bot.send_message(call.message.chat.id, "📝 مركز التعديل الشامل. ماذا تريد أن تُعدل؟", reply_markup=markup)
...
def hub_edit_cats_list(call):
    ...
    markup = types.InlineKeyboardMarkup()
    for c in cats:
        markup.add(types.InlineKeyboardButton(f"📁 قسم: {c['name']}", callback_data=f"edit_cname_{c['id']}"))
    markup.add(types.InlineKeyboardButton("⬅️ عودة", callback_data="adm_edit_hub"))
    bot.send_message(call.message.chat.id, "اختر القسم الذي تريد تعديل اسمه بالكامل:", reply_markup=markup)
...
@bot.callback_query_handler(func=lambda call: call.data == "hub_edit_msgs")
def hub_edit_msgs_list(call):
    delete_msg_safe(call.message.chat.id, call.message.message_id)
    conn = get_db()
    msgs = conn.execute("SELECT m.id, m.content, m.type, c.name as cname FROM messages m JOIN categories c ON m.cat_id = c.id").fetchall()
    conn.close()
    
    markup = types.InlineKeyboardMarkup()
    for m in msgs:
        display = f"[{m['cname']}] " + (m['content'][:20] if m['type']=='text' else "تطبيق APK / تحويل")
        markup.add(types.InlineKeyboardButton(display, callback_data=f"select_m_hub_{m['id']}"))
    markup.add(types.InlineKeyboardButton("⬅️ عودة", callback_data="adm_edit_hub"))
    bot.send_message(call.message.chat.id, "اختر الرسالة أو ملف الـ APK المرفوع الذي تريد تعديل عناصره:", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data.startswith("select_m_hub_"))
def hub_options_for_msg(call):
    delete_msg_safe(call.message.chat.id, call.message.message_id)
    msg_id = int(call.data.replace("select_m_hub_", ""))
    
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✍️ تعديل نص الرسالة أو ملف الـ APK", callback_data=f"subedit_content_{msg_id}"))
    markup.add(types.InlineKeyboardButton("🎥 تعديل رابط شرح الملف الحالي", callback_data=f"subedit_yt_{msg_id}"))
    markup.add(types.InlineKeyboardButton("🛠️ تعديل رابط شرح التثبيت الحالي", callback_data=f"subedit_inst_{msg_id}"))
    markup.add(types.InlineKeyboardButton("⬅️ إلغاء والعودة للخلف", callback_data="hub_edit_msgs"))
    bot.send_message(call.message.chat.id, "ما العنصر المحدد الذي تريد تعديله داخل هذا الملف؟", reply_markup=markup)
...
def adm_view_reviews_logic(call):
    ...
    text = "📊 آراء وتقييمات المتابعين الأخيرة (آخر 10 تقييمات):\n\n"
    if not reviews:
        text += "لا توجد أي تقييمات مضافة حالياً."
    for r in reviews:
        text += f"👤 المستخدم: `{r['user_id']}`\n⭐ التقييم: {r['rating']}\n💬 الرأي: {r['text']}\n\n"
...
# --- دالة البدء والتشغيل المستمر ---
def start_bot_polling():
    while True:
        try:
            print("🤖 [تيرمكس] بوت مزاجي المطور يعمل الآن بنجاح وبأعلى كفاءة...")
            bot.infinity_polling(timeout=20, long_polling_timeout=20, skip_pending=True)
        except Exception as e:
            print(f"⚠️ إعادة اتصال تلقائي خلال 5 ثوانٍ: {e}")
            time.sleep(5)
